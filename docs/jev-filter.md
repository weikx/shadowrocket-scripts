...
当前策略同时包含过滤信号和保留信号：直播卡片和小红书显式标记的广告固定过滤；普通帖子综合判断标题、Feed 返回的正文、分类和内容类型，不参考作者身份。Jev 分别返回内容价值、六个过滤信号，以及经验分享、生活分享和摄影分享三个保留信号，Worker 再应用可见的阈值。详细规则见下表。
...
任何一个保留信号达到阈值，都会阻止“信息价值低、情绪宣泄、负面噪音、互动诱导”这四类软规则删除该帖子。明确广告、直播、屏蔽主题、商业营销、引战和群体对立仍然可以删除，不能通过挂上“生活分享”标签逃避过滤。
...
情绪宣泄、负面噪音和互动诱导还会检查信息价值，避免把有事实、有分析或有行动建议的负面新闻、风险提示、诈骗预警和疾病科普误删。只有标题和正文同时为空时才直接标记为无内容；标题为空但正文存在时仍由 Jev 判断。
...
当前实现遵循 TypeSafe 的 System One 方式：把帖子字段组织为结构化 `state`，用一个 `Score` 判断信息价值，用多个原子 `Noul` 分别判断六个过滤信号、三个保留信号和可选屏蔽主题，再由 Worker 中的明确阈值组合最终结果。所有问题在一次请求中并行计算。模型固定为 `jev-1.13.0`，避免模型别名升级后让已经调好的阈值无提示漂移。
